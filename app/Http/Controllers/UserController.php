...
class UserController extends Controller
{
    ...
    {
        $this->middleware('auth:api' , ['except' => ['pay_sucess','pay_error','excute_pay','my_account','ads_balance','resetforgettenpassword' , 'checkphoneexistance' , 'getownerprofile']]);
    }
...
//nasser code
    ...
    public function my_data(Request $request){
        $user = auth()->user();
        $user_data = User::where('id',$user->id)->select('image','work_time_from','work_time_to','work_day_from','work_day_to')->first();
        $data['user_name'] = $user_data->name;
        $data['phone'] = $user_data->phone;
        $data['my_ads'] = Product::where('user_id',$user->id)->where('deleted',0)->where('publish','Y')->get()->count();
        $data['my_fav'] = Favorite::where('user_id',$user->id)->get()->count();
